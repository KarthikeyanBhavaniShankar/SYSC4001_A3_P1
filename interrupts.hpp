...
struct PCB{
    ...
    // Added fields (no new comments, following template style)
    unsigned int    priority;
    unsigned int    cpu_executed;
...
PCB add_process(std::vector<std::string> tokens) {
    ...
    // EXPECTS 6 or 7 fields
    // 6 fields → no priority provided → use PID
    // 7 fields → last field is priority
    if (tokens.size() < 6) {
        std::cerr << "ERROR: input line has too few fields!\n";
        exit(1);
    }
